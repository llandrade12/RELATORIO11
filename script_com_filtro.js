...
function extractData(rows) {
    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        if (dataRefPeriodo && dataRefPeriodo.getDate() <= CUTOFF_DAY) {
            p.emprestadoAte11 += valorEmprestado;
            p.previstoAte11 += valorPrevisto;
            p.pagoAte11 += valorPago;
            p.lucroRealAte11 += (valorPago - valorPrevisto); // [NOVO] Lucro Real até dia 11
        }
...
function createEmptyPeriod() {
    return {
        ...
        previstoAte11: 0, 
        pagoAte11: 0,
        lucroRealAte11: 0, // [NOVO] Lucro Real até dia 11
        status: { 
            'ATIVO': 0, 
            'NÃO EMPRESTAR': 0, 
            'QUITADO': 0, 
...
function renderConsolidatedReport(container) {
    ...
    allStatesData.forEach(state => {
        Object.entries(state.monthly).forEach(([period, d]) => {
            ...
            allPeriods[period].emprestadoAte11 += d.emprestadoAte11;
            allPeriods[period].previstoAte11 += d.previstoAte11;
            allPeriods[period].pagoAte11 += d.pagoAte11;
            allPeriods[period].lucroRealAte11 += d.lucroRealAte11;
    ...
    Object.values(allPeriods).forEach(d => {
        ...
        globalTotals.emprestadoAte11 += d.emprestadoAte11;
        globalTotals.previstoAte11 += d.previstoAte11;
        globalTotals.pagoAte11 += d.pagoAte11;
        globalTotals.lucroRealAte11 += d.lucroRealAte11;
    ...
    filteredPeriods.forEach(p => {
        ...
        filteredTotals.emprestadoAte11 += d.emprestadoAte11;
        filteredTotals.previstoAte11 += d.previstoAte11;
        filteredTotals.pagoAte11 += d.pagoAte11;
        ...
        <div class="table-container">
            <table>
                <thead>
                    <tr>
                        <th>Período</th>
                        <th class="text-right">Emprest. (Até 11)</th>
                        <th class="text-right">Previsto (Até 11)</th>
                        <th class="text-right">Pago (Até 11)</th>
                        <th class="text-right">Lucro Real (Até 11)</th>
                ...
                <tbody>
                    ${filteredPeriods.map(p => {
                        ...
                        return `
                            <tr>
                                ...
                                <td class="text-right">${formatCurrency(d.emprestadoAte11)}</td>
                                <td class="text-right">${formatCurrency(d.previstoAte11)}</td>
                                <td class="text-right">${formatCurrency(d.pagoAte11)}</td>
                                <td class="text-right ${d.lucroRealAte11 > 0 ? 'text-success' : (d.lucroRealAte11 < 0 ? 'text-danger' : '')}">${formatCurrency(d.lucroRealAte11)}</td>
                    ...
                    <tr class="totals-row">
                        ...
                        <td class="text-right">${formatCurrency(filteredTotals.emprestadoAte11)}</td>
                        <td class="text-right">${formatCurrency(filteredTotals.previstoAte11)}</td>
                        <td class="text-right">${formatCurrency(filteredTotals.pagoAte11)}</td>
                        <td class="text-right">${formatCurrency(filteredTotals.lucroRealAte11)}</td>
...
function renderStateReport(state, type, container) {
    ...
    filteredPeriods.forEach(p => {
        ...
        totals.emprestadoAte11 += d.emprestadoAte11;
        totals.previstoAte11 += d.previstoAte11;
        totals.pagoAte11 += d.pagoAte11;
        totals.lucroRealAte11 += d.lucroRealAte11;
        ...
        <div class="table-container">
            <table>
                ...
                <tbody>
                    ...
                    <tr class="totals-row">
                        <td>TOTAL FILTRADO</td>
                        <td class="text-right">${formatCurrency(totals.emprestadoAte11)}</td>
                        <td class="text-right">${formatCurrency(totals.previstoAte11)}</td>
                        <td class="text-right">${formatCurrency(totals.pagoAte11)}</td>
                        <td class="text-right">${formatCurrency(totals.lucroRealAte11)}</td>
